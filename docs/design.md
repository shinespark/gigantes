...
macOS のカメラ使用状態を検知し、Philips Hue ランプの色を変更することで「オンラインミーティング中」であることを家族に知らせるメニューバー常駐アプリ。
...
- 検知したら、指定した Hue ランプを「ON AIR 色」(例: 赤)に変更する
- カメラ使用が終了したら、ランプを**会議開始前の状態に復元**する(on/off・色・輝度)
...
- マイク使用状態の検知(音声のみ会議への対応)
- 複数ランプ / Hue シーンへの対応
- 手動オーバーライド(メニューバーから強制 ON AIR / 解除)
...
| 遷移 | アクション |
|---|---|
| idle → onAir | ① 対象ランプの現在状態を GET しスナップショット保存 → ② ON AIR 色に変更 |
| onAir → idle | スナップショットの状態をランプに書き戻し → スナップショット破棄 |
...
2. **ペアリング**: ユーザーにブリッジのリンクボタンを押してもらい、`POST /api` で application key を取得
   - body: `{"devicetype": "gigantes#<hostname>", "generateclientkey": true}`
...
- 目的: 会議中にアプリがクラッシュ / 再起動した場合、起動時に「復元し損ねたスナップショット」を検出してランプを復元できるようにするため
- 復元完了時に破棄する
...
| ケース | 方針 |
|---|---|
| 会議中にランプを手動操作された | 会議終了時に**問答無用でスナップショットへ復元**する(README に明記) |
...
| 対象ランプが物理的に電源 OFF | API エラーをメニューバーに表示、状態機械は継続 |
...
├── Gigantes/
│   ├── App/
│   │   ├── GigantesApp.swift        # @main, MenuBarExtra
...
| バージョン | 内容 |
|---|---|
| v0.1 | カメラ検知 + 単一ランプ制御 + 復元。設定は最小限 |
| v0.2 | オンボーディング整備、ログイン時自動起動、エラー表示 |
| v0.3 | 署名・notarization、GitHub Releases 配布 |
| v1.0 | README / ドキュメント整備、公開 |
| v2.x | マイク検知(CompositeDetector)、複数ランプ / シーン対応、手動オーバーライド |
