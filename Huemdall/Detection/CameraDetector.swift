...
final class CameraDetector: ActivityDetector, @unchecked Sendable {
    private static let logger = Logger(subsystem: "dev.shinespark.huemdall", category: "CameraDetector")

    /// 全ミュータブル状態はこの serial queue 上でのみ触る(リスナーブロックも同じ queue で呼ばれる)
    private let queue = DispatchQueue(label: "dev.shinespark.huemdall.camera-detector")
